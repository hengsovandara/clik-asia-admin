...
module.exports = {
  ...
  add: async (values) => {
    ...
    const entry = await Job.create(data);
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Job.associations.map(a => a.alias));
    const data = _.omit(values, Job.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Job.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Job.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Job.attributes).reduce((acc, curr) => {
      switch (Job.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
